...
class BaseTool(metaclass=ABCMeta):
    ...
    def batch_inference(self, imgs: np.ndarray):
        """Batch inference model.

        Args:
            imgs (np.ndarray): Input images in shape (N, H, W, 3).

        Returns:
            outputs (list): List of outputs for each image.
        """
        if len(imgs.shape) == 3:
            imgs = imgs[None, ...]  # (1, H, W, 3)
        
        # (N, H, W, 3) -> (N, 3, H, W)
        batch_input = imgs.transpose(0, 3, 1, 2)
        batch_input = np.ascontiguousarray(batch_input, dtype=np.float32)

        # run model
        if self.backend == 'opencv':
            raise NotImplementedError("Batch inference not supported for OpenCV backend")
        elif self.backend == 'onnxruntime':
            sess_input = {self.session.get_inputs()[0].name: batch_input}
            sess_output = []
            for out in self.session.get_outputs():
                sess_output.append(out.name)
            ...
            results = self.compiled_model(batch_input)
            output0 = results[self.output_layer0]
            output1 = results[self.output_layer1]
            outputs = [output0, output1]
        else:
