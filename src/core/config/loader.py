...
class ConfigManager:
    ...
    def __init__(self, config_path: Union[str, Path]):
        ...
        self._cache: Dict[str, SkeletonExtractionConfig] = {}
        self._raw_cache: Dict[str, Dict[str, Any]] = {}
    
    def get(self, mode: str, validate: bool = True) -> Union[SkeletonExtractionConfig, Dict[str, Any]]:
        """
        取得配置
        
        Args:
            mode: 配置模式
            validate: 是否使用 Pydantic 驗證
        ...
        if validate:
            if mode not in self._cache:
                self._cache[mode] = load_config(self.config_path, mode)
            return self._cache[mode]
        else:
            if mode not in self._raw_cache:
                self._raw_cache[mode] = get_raw_config(self.config_path, mode)
            return self._raw_cache[mode]
    
    def reload(self, mode: Optional[str] = None):
        """
        重新載入配置
        
        Args:
            mode: 指定模式，若為 None 則清除所有快取
        """
        if mode:
            self._cache.pop(mode, None)
            self._raw_cache.pop(mode, None)
        else:
            self._cache.clear()
            self._raw_cache.clear()
        
        logger.info(f"配置快取已清除: {mode or 'all'}")
